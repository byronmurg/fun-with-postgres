...
-- Named paramaters are a great way to make postgresql function calls clearer
...
-- Lets create two trivial functions to demonstrate.
...
-- Because the paramaters are swapped postgres knows which function
-- to call. Seems weird? I know.


-- Also beware when using paramater defaults. The following two functions 
...
-- throw an exception when you call say_hi with only one text paramater.



-- Don't forget that you have to drop functions with the paramater types.
...
-- Now let's tie this into our application.
...
-- the paramater will update if the table column does
